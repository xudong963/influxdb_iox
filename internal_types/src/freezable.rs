...
/// use std::sync::Mutex;
///
/// let lockable = Mutex::new(Freezable::new(23));
///
/// // Start transaction
/// let handle = {
///     let mut locked = lockable.lock().unwrap();
///     locked.try_freeze().expect("other transaction in progress")
/// };
///
/// // The contained data cannot be modified
/// assert!(lockable.lock().unwrap().get_mut().is_none());
/// // But it can still be read
/// assert_eq!(**lockable.lock().unwrap(), 23);
///
/// // --------------
/// // Do async work
/// // --------------
